 ...
 */
 ...
 public void reinas (ArrayList<int[]> soluciones){
     ...
     for (int i=0; i<tam;i++)
         solucion[i] = -1;
     reinasVueltaAtras_TodasLasSoluciones(solucion,0,soluciones);
  ...
  * @param sols
  */
 private boolean reinasVueltaAtras(int[] sol, int fila, ArrayList<int[]> sols){
     for (int i=0; i<tam; i++){
         sol[fila] = i;
         if (!hayConflicto(sol,fila))
             if (fila==tam-1){
                ...
                if (reinasVueltaAtras(sol,fila+1,sols)) {
                	return true;
                };
                //Nota: ¿por qué no funciona simplemente return reinasVueltaAtras(sol,fila+1,sols)?
     }
     return false;
 }
